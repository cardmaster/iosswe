...
int rel_geo(int planet, double rau)
{
  ...
  if (planet == MEAN_NODE || planet == TRUE_NODE || planet == LILITH) {
    ...
    rgeo = (typeof(rgeo)) (1000 * (1.0 - (rau - rmima[planet][0]) / (rmima[planet][1] - rmima[planet][0])) );
  }
  if (rgeo < 0)
    rgeo = 0;
  else if (rgeo > 999)
...
int InpHouseNr2 (struct houses *hsp, CSEC pp, CSEC *coff)
{
  ...
  if (myoff[0] == 1) {
    for (i = 1; i < 13; i++) {
      j = i + 1;
      if (j > 12) j = 1;
      myoff[j] = (typeof(myoff[0])) (swe_degnorm((hsp->cusp[j] - hsp->cusp[i]) / 360000.0) / 30.0 * myoff[j]);
    }
  }
